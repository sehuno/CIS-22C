...
class Vertex<E>
{
   ...
   public void addToAdjList(Vertex<E> neighbor, double cost)
   {
     // original
      ...
      // double possibleCost = cost;
      // Pair<Vertex<E>, Double> pair;

      // iter = adjList.entrySet().iterator();
      // while(iter.hasNext())
      // {
      //   pair = iter.next().getValue();
      //   if(neighbor.equals(pair.first) && pair.second >= possibleCost)
      //     possibleCost = pair.second + 1.0;
      // }
      // adjList.put(neighbor.data, new Pair<Vertex<E>, Double> (neighbor, possibleCost) );
   }
   ...
   // Function added to check that the degree of the vertex is even
   public boolean evenVertex()
   {
     return adjList.size() % 2 == 0;
   } 
